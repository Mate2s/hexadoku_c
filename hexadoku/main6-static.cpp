...
void vyplnpompole3(bool helf[][16][16],unsigned int delka,char pole[][16]);
int main()
{
...
void vyplnpompole3(bool helf[][16][16],unsigned int delka,char pole[][16])
{
    for(unsigned int i=0;i<delka;i++)
    {
        ...
        for(unsigned int j=0;j<delka;j++)
        {
            ...
            pomprvky[j]=pom;
            pomprvkysloupce[j]=pomsloupce;
        }
        for(unsigned int j=0;j<delka;j++)
        {
            for(unsigned int k=1+j;k<delka;k++)
            {
                if(pomprvky[j]==pomprvky[k])
                {
                    if(pomprvky[j]==2)
                    {
                        unsigned int pocet=0;
                        int prvky[2];
                        for(unsigned int f=0;f<delka;f++)
                        {
                            if(helf[i][j][f]==1 && helf[i][k][f]==1)
                            {
                                prvky[pocet]=f;
                                pocet++;
                            }
                        ...
                        if(pocet==2)
                        {
                            for(unsigned int r=0;r<delka;r++)
                            {
                                if(r!=j && r!=k)
                                {
                                    helf[i][r][prvky[0]]=false;
                                    helf[i][r][prvky[1]]=false;
                                }
                            }
                        }
                    }
                    if(pomprvky[j]==3)
                    {
                        for(unsigned int l=1+j;l<delka;l++)
                        {
                            if(l!=k)
                            {
                                if(pomprvky[j]==pomprvky[l])
                                {
                                    unsigned int pocet=0;
                                    int prvky[3];
                                    for(unsigned int f=0;f<delka;f++)
                                    {
                                        if(helf[i][j][f]==1 && helf[i][k][f]==1 && helf[i][l][f]==1)
                                        {
                                            prvky[pocet]=f;
                                            pocet++;
                                        }
                ...
                if(pomprvkysloupce[j]==pomprvkysloupce[k])
                {
                    if(pomprvkysloupce[j]==2)
                    {
                        unsigned int pocet=0;
                        int prvky[2];
                        for(unsigned int f=0;f<delka;f++)
                        {
                            if(helf[j][i][f]==1 && helf[k][i][f]==1)
                            {
                                prvky[pocet]=f;
                                pocet++;
                            }
                        ...
                        if(pocet==2)
                        {
                            for(unsigned int r=0;r<delka;r++)
                            {
                                if(r!=j && r!=k)
                                {
                                    helf[r][i][prvky[0]]=false;
                                    helf[r][i][prvky[1]]=false;
                                }
                            }
                        }
                    }
                    if(pomprvkysloupce[j]==3)
                    {
                        for(unsigned int l=1+j;l<delka;l++)
                        {
                            if(l!=k)
                            {
                                if(pomprvkysloupce[j]==pomprvkysloupce[l])
                                {
                                    unsigned int pocet=0;
                                    int prvky[3];
                                    for(unsigned int f=0;f<delka;f++)
                                    {
                                        if(helf[j][i][f]==1 && helf[k][i][f]==1 && helf[l][i][f]==1)
                                        {
                                            prvky[pocet]=f;
                                            pocet++;
                                        }
    ...
    for(unsigned int a=0;a<4;a++)
    {
        for(unsigned int b=0;b<4;b++)
        {
            ...
            for(unsigned int c=0;c<4;c++)
            {
                for(unsigned int d=0;d<4;d++)
                {
                    for(unsigned int k=1+c*4+d;k<delka;k++)
                    {
                        if(pomprvky[c*4+d]==pomprvky[k])
                        {
                            if(pomprvky[k]==2)
                            {
                                unsigned int pocet=0;
                                ...
                                for(unsigned int f=0;f<delka;f++)
                                {
                                    if(helf[a*4+c][b*4+d][f]==1 && helf[a*4+k/4][b*4+k%4][f]==1)
                                    {
                                        prvky[pocet]=f;
                                        pocet++;
                                    }
                                ...
                                if(pocet==2)
                                {
                                    for(unsigned int r1=a*4;r1<a*4+4;r1++)
                                    {
                                         for(unsigned int r2=b*4;r2<b*4+4;r2++)
                                        {
                                        if((r1!=a*4+c && r2!=b*4+d) && (r1!=a*4+k/4 && r2!=b*4+k%4))
                                        {
                                            helf[r1][r2][prvky[0]]=false;
                                            helf[r1][r2][prvky[1]]=false;
                                        }
